...
FROM
    users
WHERE
    llaveRol_usuario = 13  OR llaveRol_usuario = 12";
$result = mysqli_query($conexion1, $sql);
while ($resultado = mysqli_fetch_array($result)) {
    ?>

                ['<b><?php echo $resultado["name"] ?></b>', 25],
...
WHERE
    sbc.sub_estados = 'AVALADO'    and id = $id
GROUP BY id";
    $cuantosCali = 0;
...
WHERE
    sbrcl.sub_estados = 'AVALADO'  and id = $id and date(fechaCreacionCaso) between  '$desde' and '$hasta'
GROUP BY id ";
...
WHERE
    sbpcl.sub_estados = 'AVALADO'  and id = $id and date(fechaCreacionCaso) between  '$desde' and '$hasta'
GROUP BY id ";
...
WHERE
    sbstadc.sub_estados = 'AVALADO'  and id = $id and date(fechaCreacioonAdicin) between  '$desde' and '$hasta'
GROUP BY id ";
...
FROM
   tbl_adicionpcls AS ad
    ...
    users AS ucl ON ucl.id = ca.llaveCalificadorCalifiacion
WHERE
    sbc.sub_estados = 'AVALADO'  and id = $id and date(fechaCreacioonAdicin) between  '$desde' and '$hasta'
GROUP BY id ";
...
FROM
   tbl_adicionpcls AS ad
    ...
    users AS urc ON urc.id = rc.llaveCalificadorRecalificacion
WHERE
    sbrcl.sub_estados = 'AVALADO'  and id = $id and date(fechaCreacioonAdicin) between  '$desde' and '$hasta'
GROUP BY id ";
    ...
    ?>
                ['<b><?php echo $resultado["name"] ?></b>',<?php echo $cuantosCali + $cuantosReCali + $cuantosAdi + $cuantosAdiCa + $cuantosAdiSolo + $cuantosPrecaRe ?> ],
...
<?php
$cuenta = 0;
$array = array('#C8E6C9', '#FFE0B2 ', '#A5D6A7', '#FFB74D', '#C8E6C9');
...
while ($resultado = mysqli_fetch_array($result)) {
    ...
    ?>
            {
                name: '<b><?php echo $resultado["name"] ?></b>',
                color: '<?php echo $array[$cuenta] ?>'
...
?>
    ...
    var chart = Highcharts.chart('container', {
        ...
        title: {
            text: 'Medición de la productividad (profesionales de planta) entre casos asignados Vrs Gestionados(avalados).'
        },
        subtitle: {
//            text: 'Comparing to results from Summer Olympics 2012 - Source: <ahref="https://en.wikipedia.org/wiki/2016_Summer_Olympics_medal_table">Wikipedia</a>'
        ...
        tooltip: {
            ...
            pointFormat: '<span style="color:{point.color}">\u25CF</span> {series.name}: <b>{point.y} Siniestros</b><br/>'
        },
        xAxis: {
            type: 'category',
        ...
        exporting: {
            filename: 'Asignados Vrs Gestionados(avalados).',
            sourceWidth: 800,
            sourceHeight: 600,
            // scale: 2 (default)
            chartOptions: {
