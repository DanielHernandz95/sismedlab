...
<div class="card-body">
    <div id="volumenQuienSolicita"></div>
</div>
<script>
<?php
...
$quienSolicita = $_GET['quienSolicita'];

?>
    Highcharts.chart('volumenQuienSolicita', {
        chart: {
        ...
        title: {
            text: 'Casos radicados por quien solicita ( <?php echo $quienSolicita = $_GET['quienSolicita'];?> )  POR MES'
        },
...
WHERE
    ec.estado_siniestro = '$estado'   and date(fechaCreacionCaso) between  '$desde' and '$hasta' and quien_solicita = '$quienSolicita'
GROUP BY id";
    $cuantosCali = 0;
...
WHERE
    ercl.estado_siniestro = '$estado' and date(fechaCreacionCaso) between  '$desde' and '$hasta' and quien_solicita = '$quienSolicita'
GROUP BY id ";
...
WHERE
    epc.estado_siniestro = '$estado' and date(fechaCreacionCaso) between  '$desde' and '$hasta' and quien_solicita = '$quienSolicita'
GROUP BY id ";
...
WHERE
    edc.estado_siniestro = '$estado' and date(fechaCreacioonAdicin) between  '$desde' and '$hasta' and quien_solicita = '$quienSolicita'
GROUP BY id ";
...
FROM
   ...
   INNER JOIN
    ...
    users AS ucl ON ucl.id = ca.llaveCalificadorCalifiacion
WHERE
    ec.estado_siniestro = '$estado' and date(fechaCreacioonAdicin) between  '$desde' and '$hasta' and quien_solicita = '$quienSolicita'
GROUP BY id ";
...
FROM
   ...
   INNER JOIN
    ...
    users AS urc ON urc.id = rc.llaveCalificadorRecalificacion
WHERE
    ercl.estado_siniestro = '$estado' and date(fechaCreacioonAdicin) between  '$desde' and '$hasta'  and quien_solicita = '$quienSolicita'
GROUP BY id ";
    ...
    if (($cuantosCali + $cuantosReCali + $cuantosAdi + $cuantosAdiCa + $cuantosAdiSolo + $cuantosPrecaRe) > 0) {
        ?>

                            {

                                name: '<?php echo $estado ?>',
                                y: <?php echo $cuantosCali + $cuantosReCali + $cuantosAdi + $cuantosAdiCa + $cuantosAdiSolo + $cuantosPrecaRe ?>
                            },
        <?php
    }
}
?>
                    ...
                    exporting: {
            filename: 'Volúmenes de radicación por Quien solicita',
            sourceWidth: 800,
            sourceHeight: 600,
            // scale: 2 (default)
            chartOptions: {
